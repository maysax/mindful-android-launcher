...
//  SKIP          savedInstanceState.putInt(KEY_THEME_COLOR, mColorFilter.getColor());
        }
    }
    ...
    private static Entry dataItemToEntry(DataItem dataItem,
        ...
        if (dataItem instanceof ImDataItem) {
            ...
            entryContextMenuInfo = new EntryContextMenuInfo(subHeader, header,
                    dataItem.getMimeType(), dataItem.getId(), dataItem.isSuperPrimary());
            try {
//                final WebAddress webAddress = new WebAddress(website.buildDataString(context, kind));
//                intent = new Intent(Intent.ACTION_VIEW, Uri.parse(webAddress.toString()));
                Tracer.i("WebAddress url: " + website.buildDataString(context, kind));
